...
class TestRegistryConfig(TransactionCase):
    def setUp(self):
        super(TestRegistryConfig, self).setUp()
    ...
    def test_set_values(self, mock_post):
        ...
        self.registry_config.id_generator_auth_url = "https://keycloak.openg2p.org/realms/master/protocol/openid-connect/token"
        ...
        self.assertEqual(config.auth_url, "https://keycloak.openg2p.org/realms/master/protocol/openid-connect/token")
    ...
    def test_add_missing_ref_id_to_retry(self, mock_post):
        ...
        partner_with_ref = self.env['res.partner'].create({
        ...
        pending_records = self.env["g2p.pending.reference_id"].search([('registrant_id', '=', partner_without_ref.id)])
